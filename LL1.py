...
def validateStringUsingStackBuffer(parsing_table, grammarll1,
	...
	while True:
		...
		if stack == ['$'] and buffer == ['$']:
			...
			if parsing_table[x][y] != '':   # STOP HERE STOP HERE
				array_of_actions.append(parsing_table[x][y])
			...
			if stack[0] == buffer[-1]:
				array_of_actions.append("Matched")
				print("{:50} {:60} {:70}"
					.format(' '.join(buffer),
							' '.join(stack),
							f"Matched:{stack[0]}"))
...
for i in range(len(input1)):
	if input1[i][0].isalpha():
		input1[i] = 'id'
	elif input1[i][0].isnumeric():
		input1[i] = 'num'


string1 = ' '.join([str(item) for item in input1])
...
with open("Output4.txt", "w") as a_file:
	for x in array_of_actions:
		a_file.write(x)
		a_file.write("\n")
